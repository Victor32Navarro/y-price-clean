...
const SEARCH_RESULT_LIMIT = 5; // kolik stránek necháme scrapnout Firecrawlem
                                // (méně = rychlejší odpověď, míň riziko timeoutu)
...
function buildScrapedContext(results) {
  return results
    .map((item, i) => {
      ...
      const url = item.url || '';
      const content = (item.markdown || item.description || '').slice(0, 2000);
      return `--- ZDROJ ${i + 1} ---\nNÁZEV STRÁNKY: ${title}\nURL: ${url}\nOBSAH:\n${content}`;
    })
    .join('\n\n');
}
